			...
			<div class="col-sm-9">
				...
				<div class="border border-secondary p-4 rounded">
					<div class="row">
						<div class="col-sm-6 ">
							<h3 style="font-family: 'Noto Sans KR', sans-serif;">
								커뮤니티 <small>(글개수 : ${ pageMaker.totalCount })</small>
							</h3>
						</div>
						<div class="col-sm-6">
							<!-- 새글쓰기 버튼 -->
							<c:if test="${ not empty sessionScope.memberId }">
								<button type="button"
									class="btn btn-secondary btn-sm float-right my-3 "
									onclick="location.href = '/community/commuBoardWrite?pageNum=${ pageMaker.cri.pageNum }';">
									<i class="material-icons align-middle">create</i> <span
										class="align-middle Board-font">글쓰기</span>
								</button>
							</c:if>
						</div>
					</div>
									...
									<c:forEach var="board" items="${ commuList }">
										<tr>
											...
											<td><a class="align-middle"
												href="/community/commuBoardContent?boardId=${ board.boardId }&pageNum=${ pageMaker.cri.pageNum }">${ board.boardTitle }</a>
											</td>
					...
					<nav aria-label="Page navigation example">
						<ul class="pagination justify-content-center">
							...
							<li class="page-item ${(pageMaker.prev) ? '': 'disabled'}">
								<a class="page-link"
								href="${(pageMaker.prev) ? '/community/commuBoardList?pageNum=' += (pageMaker.startPage - 1) += '&type=' += pageMaker.cri.type += '&keyword=' += pageMaker.cri.keyword : '' }#board"
							...
							<c:forEach var="i" begin="${ pageMaker.startPage }"
								...
								<li
									...
									<a class="page-link"
									href="/community/commuBoardList?pageNum=${ i }&type=${ pageMaker.cri.type }&keyword=${ pageMaker.cri.keyword }#board">${ i }</a>
								</li>
							</c:forEach>
					...
					<form class="form-inline justify-content-center my-4"
						action="/community/commuBoardList" method="get">
